...
/// @dev The gen-8 seam. Asynchronous by necessity — VRF is request/callback —
///      which is exactly why this costs a board generation rather than an
///      entropy redeploy: the old `IEntropy` was all `view`, and a view cannot
///      wait for an oracle.
interface IAsyncEntropy {
    ...
    function draws(bytes32 salt) external view returns (uint256 requestId, uint256 word, uint64 requestedAt, bool ready);
}
...
/**
 * @title SegmentBoardVRF9
 * @notice Generation 9 of the SwapTables board. Identical to gen-8 (VRF entropy)
 *         but for one economic change: the table seed no longer enters any pool.
 *
 * ## Why gen-9 exists
 *
 * Gen-8's §9 gate gave a contested pool a share of the 100-TIMBS house seed once
 * it had two distinct wallets. Two Sybil wallets are free on a permissionless
 * chain, and hedging Red against Black makes one of the operator's OWN wallets
 * win every pool — so the seed share was harvested risk-free, ~79% of every
 * table's seed (dev-docs/AUDIT_SEED_FARM.md, tests/SeedFarmExploit.t.sol). No
 * distinct-wallet threshold closes this: a gate of N costs the attacker N
 * wallets.
 *
 * Gen-9 removes the thing the lever was attached to. `_potOf` adds no seed share
 * — pots are only the chips genuinely at risk — and `retire` routes the whole
 * untouched seed to the UnderwriteReserve. The house still sweetens play, now
 * exclusively through the capped underwrite, which pays real winners strictly
 * below fair odds and therefore cannot be farmed. Honest winners are unaffected:
 * the top-up backstops them to `stake x fair x 0.90` either way.
 *
 * Everything else — VRF entropy, seats, loads, bets, the monotonic underwrite,
 * dealer tips, the bonus-chip rule, the retire waterfall's rake split — is gen-8
 * unchanged. `SegmentBoard` (gen 4-7) and `SegmentBoardVRF` (gen 8) remain in the
 * tree; the seed change was too economically load-bearing to edit into a live,
 * verified board, so it ships as a fresh generation exactly as the others did.
 ...
 *
 * Entropy — GENERATION 8, the change that defines this board (docs/GEN8_VRF.md):
 *   There are no commitments and no secrets. After the pick time anyone arms ONE
 *   segment at a time; each arm fires one Chainlink VRF request, and when that
 *   word lands anyone may lock that segment. The char is
 *   `keccak(vrfWord, salt) % 36`.
 *
 *   Why the rewrite. Generations 1-7 let the wallet holding the secrets compute
 *   BOTH the reveal outcome and the 64-block fallback outcome once the lock
 *   block was public, then pick between them by acting or not acting — a Colour
 *   bet worth 50% honestly became 75% (docs/ENTROPY_TRUST.md). With no secret
 *   there is no second branch, so there is nothing to select. The fix is
 *   structural: the lever is not policed, it does not exist.
 *
 *   Why one request per segment. A fulfilled word is public the instant the
 *   callback lands, so requesting six up front would publish the whole round at
 *   once and kill the staggered drumroll. Arming segment by segment means each
 *   char becomes knowable only when its own draw lands — and at that moment
 *   locking is permissionless, so no party ever knows more than the room.
 *
 *   The stall escape is `rerequest`, not a second derivation path. Re-drawing an
 *   UNFULFILLED request is a choice between two unknowables, which is no choice
 *   at all; a fulfilled draw can never be replaced.
 *
 * Status:
 *   Generations 1-7 ran commit-reveal. Generation 1 ran a full round live on
 *   Arbitrum Sepolia (2026-07-26): open -> seat -> load -> place -> arm -> six
 *   locks -> retire -> withdraw, with the ledger draining to exactly zero.
 *   Escrow conservation, the graduated rake (4.87% at two wallets vs 8% solo)
 *   and the §9 seed guard all reconciled to the wei. See
 *   SwapTables/docs/VALIDATION.md. Everything below the entropy seam — seats,
 *   loads, bets, the pari-mutuel settle, the gen-6 underwrite and retire
 *   waterfall, gen-6 tips, the gen-7 bonus-chip rule — is that same code.
 *
 *   A table that never reaches SEATS_MIN can never be armed, so it would never
 *   lock or retire and its loaded chips would be stranded. cancelTable() closes
 *   it — permissionless, refunds every chip, returns the seed.
 *
 * Security:
 ...
 */
contract SegmentBoardVRF9 is Ownable, ReentrancyGuard {
    // ─── Constants ─────────────────────────────────────────────────────────────
    ...
    // Gen-9: SEED_MIN_WALLETS now gates ONLY the rake (an uncontested pool is
    // never raked, §8/Layer-0). It no longer gates a seed share, because gen-9
    // puts no seed in any pot — see _potOf and the seed reroute in retire.
    uint8 public constant SEED_MIN_WALLETS = 2;

    uint256 public constant TABLE_SEED = 100e18;              // §9
    // SEED_SHARE is gone in gen-9: no pool ever receives a share of the seed.
    // The whole TABLE_SEED is routed to the UnderwriteReserve at retire, closing
    // the two-wallet seed-farm (dev-docs/AUDIT_SEED_FARM.md). The seed still
    // sweetens play -- now only through the capped, un-farmable underwrite path.
    ...
    // Gen-8 has no REVEAL_WINDOW and no BLOCKHASH_HORIZON: nothing is derived
    // from a block hash, so nothing expires with one. A draw that never lands is
    // handled by the entropy module's own re-request delay, not by a deadline
    // here — the board deliberately holds no second way to produce a char.
    ...
    IAsyncEntropy      public immutable entropy;
    ...
    struct Table {
        uint64  openedAt;
        uint64  pickTime;
        uint64  armedAt;     // block of the FIRST segment arm; 0 until armed
        uint32  seedRound;
        uint8   seatCount;
        ...
        // ── gen-8 ──
        uint8   armedMask;    // bit i => segment i+1 has a draw requested
    }
    ...
    event SegmentArmed(uint256 indexed tableId, uint8 indexed segment, uint256 requestId);
    event SegmentRearmed(uint256 indexed tableId, uint8 indexed segment, uint256 requestId);
    /// @dev `requestId` replaces gen-1..7's `viaFallback` flag: there is no
    ///      fallback path, and naming the draw makes every char auditable back
    ///      to a specific VRF fulfilment.
    event SegmentLocked(uint256 indexed tableId, uint8 indexed segment, bytes1 lockedChar, uint256 requestId);
    ...
    constructor(
        ...
        entropy      = IAsyncEntropy(_entropy);
     ...
     *         seeds two tables.
     * @dev Gen-8 takes no commitments: there is no secret to commit to. The
     *      opener is now purely the host — it holds nothing the room does not.
     */
    function openTable(uint256 seedRound)
        external
        nonReentrant
    ...
    /**
     * @notice Arm ONE segment at/after the pick time: fires its VRF draw.
     *         Permissionless.
     *
     * @dev Segment by segment, not all six at once, and that is the whole
     *      staging of the show. A fulfilled word is public the instant its
     *      callback lands, so six requests fired together would publish the
     *      entire round in one go. Armed one at a time, each char becomes
     *      knowable only when its own draw returns — and from that instant
     *      anyone may lock it, so nobody is ever holding a result the room
     *      cannot see.
     *
     *      No ordering is imposed. A caller may arm segment 4 first; the reveal
     *      order is the auto-pilot's business, not a rule worth spending a
     *      revert on.
     */
    function armSegment(uint256 tableId, uint8 segment) external nonReentrant {
        Table storage t = _liveTable(tableId);
        if (segment == 0 || segment > SEGMENTS) revert BadSegment();
        ...
        uint8 bit = uint8(1) << (segment - 1);
        if (t.lockedMask & bit != 0) revert SegmentAlreadyLocked(segment);
        if (t.armedMask  & bit != 0) revert SegmentAlreadyArmed(segment);

        t.armedMask |= bit;
        if (t.armedAt == 0) t.armedAt = uint64(block.number);

        uint256 requestId = entropy.requestFor(_salt(tableId, segment));
        emit SegmentArmed(tableId, segment, requestId);
    }

    /**
     * @notice Replace a draw that never came back, so a table can never jam.
     *         Permissionless.
     *
     * @dev This is gen-8's ONLY stall escape, and deliberately not a second way
     *      to produce a char. Gens 1-7 had one: the blockhash fallback, which
     *      gave the secret-holder a knowable alternative outcome and therefore a
     *      free pick between two results (docs/ENTROPY_TRUST.md). Re-drawing an
     *      *unfulfilled* request is a choice between two unknowables, which is no
     *      choice at all — and the module refuses outright once a draw is in, so
     *      a landed result can never be rerolled by anyone, including the owner.
     *
     *      The delay lives in the entropy module (REREQUEST_DELAY) because it is
     *      a property of the oracle, not of the game.
     */
    function rearmSegment(uint256 tableId, uint8 segment) external nonReentrant {
        Table storage t = _liveTable(tableId);
        if (segment == 0 || segment > SEGMENTS) revert BadSegment();
        uint8 bit = uint8(1) << (segment - 1);
        if (t.armedMask  & bit == 0) revert SegmentNotArmed(segment);
        if (t.lockedMask & bit != 0) revert SegmentAlreadyLocked(segment);

        // Reverts inside the module if the draw already landed or the delay has
        // not elapsed — the guard belongs with the draw, not duplicated here.
        uint256 requestId = entropy.rerequest(_salt(tableId, segment));
        emit SegmentRearmed(tableId, segment, requestId);
    }

    /**
     * @notice Lock a segment once its draw has landed, then settle its pool.
     *         Permissionless and argument-free — there is no secret to present,
     *         so anyone at all can turn the card over.
     */
    function lockSegment(uint256 tableId, uint8 segment) external nonReentrant {
        Table storage t = _prepLock(tableId, segment);
        bytes32 salt = _salt(tableId, segment);
        if (!entropy.isReady(salt)) revert DrawNotIn(segment);
        (uint256 requestId,,,) = entropy.draws(salt);
        _applyLock(tableId, t, segment, entropy.entropyFor(salt), requestId);
    }
    ...
    function _applyLock(
        ...
        emit SegmentLocked(tableId, segment, c, requestId);
    ...
    /// @dev Gen-9: a pot is ONLY the chips genuinely staked in the pool. No seed
    ///      share is ever added -- that share was risk-free money a winning pool
    ///      handed out, and two Sybil wallets hedging Red/Black harvested it every
    ///      table (dev-docs/AUDIT_SEED_FARM.md). With no house money in the pot
    ///      there is nothing to skim; the seed now reaches players only through
    ///      the capped underwrite, which pays real winners below fair odds and so
    ///      cannot be farmed.
    ...
    function retire(uint256 tableId) external nonReentrant {
        ...
        // Gen-9 flow of funds. Dead pots + half the rake route to the reserve as
        // before — income matched to the top-up liability by activity. What's new
        // is the SEED: gen-8 sprinkled it across contested pots (the farm surface);
        // gen-9 never put it in a pot, so the whole TABLE_SEED still sits untouched
        // in escrow at retire and is swept to the reserve too. The seed now
        // sweetens play exactly where it can't be farmed — as reserve float that
        // funds the capped underwrite. Treasury still takes the rake's other half.
        //
        // The seed reaches the reserve PHYSICALLY here (it's part of toReserve).
        // It is deliberately NOT reported through recordIncome: gameIncome stays a
        // measure of what the GAME earned (rake + dead pots), while the seed is
        // house-provided float. The reserve's waterfall keys off its balance, not
        // gameIncome, so the seed is parked into the float/overflow correctly all
        // the same. All figures are this table's own escrow; another live table's
        // stake is unreachable here.
        uint256 dead     = deadPotAccrued[tableId];
        uint256 rakeHalf = rakeAccrued[tableId] / 2;
        uint256 toReserve = dead + rakeHalf + TABLE_SEED; // gen-9: + the whole seed
    ...
    function cancelTable(uint256 tableId) external nonReentrant {
        ...
        // Unreachable while armSegment enforces SEATS_MIN, but asserted so the
        // "nothing settled" premise above can never quietly stop holding.
        if (t.armedMask != 0) revert AlreadyArmed();
    ...
    // ─── Draw views (what the apps and auto-pilot poll) ────────────────────────

    /// @notice The id the next openTable() will assign.
    function nextTableId() external view returns (uint256) {
        return tableCount + 1;
    }

    /// @notice The salt binding one table+segment to its draw. Public because
    ///         the entropy module is keyed by it — anyone can verify a char
    ///         against the fulfilled word without trusting this contract.
    function saltFor(uint256 tableId, uint8 segment) external pure returns (bytes32) {
        return _salt(tableId, segment);
    }

    /**
     * @notice Everything the stream needs about one segment in a single call:
     *         whether it is armed, whether its draw has landed, whether it can
     *         be locked right now, and whether a stuck draw is replaceable.
     */
    function segmentState(uint256 tableId, uint8 segment)
        external
        view
        returns (bool armed, bool locked, bool drawIn, bool lockable, bool replaceable)
    {
        Table storage t = tables[tableId];
        uint8 bit = uint8(1) << (segment - 1);
        armed  = t.armedMask  & bit != 0;
        locked = t.lockedMask & bit != 0;
        if (!armed) return (false, locked, false, false, false);
        bytes32 salt = _salt(tableId, segment);
        drawIn      = entropy.isReady(salt);
        lockable    = drawIn && !locked && !t.retired;
        replaceable = !locked && entropy.replaceable(salt);
    }
