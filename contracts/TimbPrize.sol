...
contract TimbPrize is Ownable, ReentrancyGuard {
    ...
    function _settleRound() internal {
        ...
        // ── Freeze position ──────────────────────────────────────────────────
        // entropy: blockhash(block.number - 1) + positionCounter + roundNumber
        ...
        // Random 3-second offset within 0:15 window (0, 1, or 2 seconds)
        uint256 offset  = entropy % 3;
        uint256 frozenCounter = positionCounter + offset; // simulate freeze offset

        bytes6 winningString = getWindowAt(frozenCounter);
        roundWinningString[round] = winningString;

        emit PositionFrozen(round, frozenCounter, winningString);

        // ── Find winners (dual-layer verification) ───────────────────────────
        ...
        if (protocolCutBps > 0 && pot > 0) {
            ...
            // Protocol cut stays in TimbTreasury (sent separately off-chain
            // or via a treasury.receiveCut() call — treasury receives from escrow)
            pot -= cut;
            emit ProtocolCutTaken(cut);
        }

        // ── Pot math: floor(x/n)*n, remainder r ──────────────────────────────
        uint256 perWinner    = 0;
        uint256 totalPaid    = 0;
        ...
        for (uint256 i = 0; i < winnerCount; i++) {
            ...
            roundWinners[round].push(verified[i]);
        }
        ...
        // ── Get total entrant count for event ─────────────────────────────────
        uint256 totalEntries =
